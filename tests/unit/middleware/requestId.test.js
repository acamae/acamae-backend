...
describe('requestIdMiddleware', () => {
  ...
  it('should handle UUID generation error gracefully', () => {
  ...
  it('should handle UUID generation error with actual fallback generation', () => {
    // Mock uuid to throw an error
    uuidv4.mockImplementationOnce(() => {
      throw new Error('UUID generation failed');
    });
    ...
    expect(console.error).toHaveBeenCalledWith('Error generating requestId:', expect.any(Error));
    expect(req.requestId).toMatch(/^\d+-[a-z0-9]{9}$/); // timestamp-random pattern
  ...
  it('should work when headers is undefined', () => {
    ...
    // This will trigger the error path since accessing undefined.key will fail
    requestIdMiddleware(req, res, next);

    expect(req.requestId).toMatch(/^\d+-[a-z0-9]{9}$/); // fallback pattern
    expect(res.setHeader).toHaveBeenCalledWith('X-Request-ID', req.requestId);
    expect(next).toHaveBeenCalled();
    expect(console.error).toHaveBeenCalledWith('Error generating requestId:', expect.any(Error));
  });
